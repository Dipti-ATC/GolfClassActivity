...
namespace GolfClass
{
    ...
    {
        //connection string
        private string connectionString = @"Data Source=VHMASTW-20\SQLEXPRESS;Initial Catalog=Golf;Integrated Security=True;";
        ...
        {
            //Scalar query to return a single value. 
            ...
            using(SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand Command = new SqlCommand(queryString, connection);
                connection.Open();
                btnCountGolfers.Text = Command.ExecuteScalar().ToString();
                connection.Close();
            }
